...
describe('public API contracts', () => {
  ...
  it('uses real counter fields and retains full names from server-side options', async () => {
    ...
    expect(updateSession).toHaveBeenLastCalledWith('123', { state: 'awaiting_counter', data: { institutionId: '1', institutionName: 'Full institution name', step: 2 } });
  });
  it('uses position and known institution scope when issuing a public ticket without IDs', async () => {
    ...
    await processJoinQueueFlow('123', { ...session, state: 'awaiting_confirmation', data: { step: 4, institutionId: '7', counterId: '8' } }, 'confirm_yes', {
    ...
    expect(updateSession).toHaveBeenCalledWith('123', { state: 'in_queue', data: { tokenNumber: 'GEN-0042', tokenId: undefined, flow: undefined, step: undefined } });
  });
});
