...
1. Empirical — the institution's own measured average service time
   ...
   This uses local data; superiority over ML has not been measured.
2. Heuristic cold start — before enough local history exists,
   people ahead × a configurable default service time.

Validated global ML output is exposed only for shadow evaluation, with an
explicit status and no numeric value when invalid. The historical artifact
contains a post-service feature and dataset-local numeric IDs; it is not used
for live numbers. See app/ml/README.md for the audit and promotion requirements.
"""
...
from app.ml.predictor import ModelEstimate, predictor
from app.models import Counter, Token, TokenStatus


def active_queue_length(db: Session, counter_id: int, institution_id: int) -> int:
...
def ml_estimate_minutes(db: Session, *, institution_id: int, counter: Counter, people_ahead: int | None = None) -> ModelEstimate:
    """Validated shadow result, never used for customer-facing estimates."""
    ...
    try:
        return predictor.evaluate(
            ...
            queue_length_at_arrival=float(active_queue_length(db, counter.id, institution_id) if people_ahead is None else people_ahead),
        )
    except Exception:
    ...
    *,
    ...
    try:
        if people_ahead is None:
            people_ahead = active_queue_length(db, counter.id, institution_id)
